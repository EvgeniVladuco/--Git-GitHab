...
После этого через поиск мжно находитьразных авторов, разные проекты.
...
Теперь переходим в *WS code*, открываем терминал и вводимследующую команду:
```
...
Если мы дальше будем работать с нашим локальным репозиториемб то все изменения, которые мы внесём, останутся только в локальном. Для того что бы они перешли в удаленный, необходимо дать эту команду Gitу.
